...
void init_common(char* config_file){
  ...
  if (rank==0){
    readConfig(config_file,&nx,&ny,&kx,&ky,&nsteps,&wstep);
  }
  /* don't really need anything below here */
...
//TODO:
//Optimizatio can be done here to ensure that only one proc
//needs to make these calculations
void divide_work(){
  int residual,xBig;
