...
float **fmatrix(int e_static,int e_cut){
  ...
  m = (float**)malloc( e_cut*sizeof(float) );
  if(!m) fprintf(stderr,"Allocation Failure, 1st dimension");
  
  for(i=0;i<e_cut;i++) {
    m[i] = (float*)malloc( e_static*sizeof(float) );
...
void  readConfig(char *infilename,int* bs,
  ...
  printf("diffusion2d: "
	 "bs=%d, nx=%lld, ny=%lld, k=%.2le, nsteps=%d, wstep=%d"
	 "delay_dur=%lf, delay_per_mean=%lf, delay_per_stddev=%lf \n",
         *bs, *nx, *ny, *k, *nsteps, *wstep,
